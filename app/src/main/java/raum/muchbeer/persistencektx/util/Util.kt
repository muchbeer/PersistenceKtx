...
fun formatNights(nights: List<SleepEntity>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach {
            ...
            append("\t${convertLongToDateString(it.startSleep)}<br>")
            if (it.stopSleep != it.startSleep) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(it.stopSleep)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(it.sleepQuality, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                // Hours
                append("\t ${it.stopSleep.minus(it.startSleep) / 1000 / 60 / 60}:")
                // Minutes
                append("${it.stopSleep.minus(it.startSleep) / 1000 / 60}:")
                // Seconds
                append("${it.stopSleep.minus(it.startSleep) / 1000}<br><br>")
            }
        }
    }
